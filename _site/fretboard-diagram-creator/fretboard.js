...
// intervals are given in semitones
var data = [
    {  
        ...
        "lowNote":          "E",
        "fretted":          true,
        "markers":  [3, 5, 7, 9, 12, 15, 17, 19, 21, 23],
    },
    {  
        "name":             "guitar - DADGAD",   
        "stringIntervals":  [24, 19, 17, 12, 7, 0],
        "lowNote":          "D",
        "fretted":          true,
        "markers":  [3, 5, 7, 9, 12, 15, 17, 19, 21, 23],
    },
    ...
    {  
        ...
        "stringIntervals":  [21, 14, 7, 0],
        "lowNote":          "G",
        "fretted":          false,
        "markers":  [2, 4, 5, 7],
    }, 
    {  
        "name":             "violin (5 string) - standard",   
        "stringIntervals":  [28, 21, 14, 7, 0],
        "lowNote":          "C",
        "fretted":          false,
        "markers":  [2, 4, 5, 7],
    }, 
...
function changed(value) {
    ...
    initialize()
    var rotationIndex = -1*notes[0].indexOf(data[i].lowNote);
    fretboard.consts.notes[0] = arrayRotate(notes[0], rotationIndex);
    fretboard.consts.notes[1] = arrayRotate(notes[1], rotationIndex);
    fretboard.consts.stringIntervals = [];
    fretboard.consts.stringIntervals = data[i].stringIntervals;
    fretboard.consts.fretted = data[i].fretted;
    fretboard.consts.markers = data[i].markers
    fretboard.erase();
    fretboard.reinitialize();
}


var selectElement = document.getElementById('sel');
selectElement.setAttribute( "onchange", "changed(selectElement.value)" );

data.map(item => selectElement.appendChild(new Option(item.name, item.id)).cloneNode(true));
...
class Fretboard {
    ...
    drawFrets() {
        ...
        if (this.consts.fretted) {
            ...
            pathSegments.push("v " + (1) * this.consts.fretHeight);
        }
    ...
    drawMarkers() {
        ...
        for (let i of filteredMarkers) {
            ...
            if (this.consts.fretted) {
                ...
                marker = createSvgElement('text', {
                    class: 'marker',
                    x: this.consts.offsetX + (i - 1 - this.state.startFret) * this.consts.fretWidth + (this.consts.fretWidth),
                    y: this.consts.offsetY + this.consts.fretHeight + this.consts.stringSpacing,
                });
            }
            marker.innerHTML = i;
    ...
    drawNotes() {
        ...
        for (let i = this.state.startFret; i < this.state.endFret; i++) {
            for (let j = 0; j < this.consts.numStrings; j++) {
                ...
                var x;
                if (this.consts.fretted) {
                    x = this.consts.offsetX + (this.consts.fretWidth / 2) + this.consts.fretWidth * (i - this.state.startFret);
                } else {
                    x = this.consts.offsetX + (this.consts.fretWidth) + this.consts.fretWidth * (i - this.state.startFret);
                }
                const y = this.consts.offsetY + this.consts.stringSpacing * j;
...
/* Save instrument list button */

var instrumentButton = document.getElementById('save-instruments');
const instrumentLink = document.getElementById('instrument-link');

instrumentButton.addEventListener('click', () => {
    fretboard.clearSelection();
    var instrumentData = JSON.stringify(data, null, 2);
    var instrumentBlob = new Blob([instrumentData], { type: "data:text/plain;charset=utf-8" });
    var instrumentURL = URL.createObjectURL(instrumentBlob);
    instrumentLink.href = instrumentURL;
    instrumentLink.click();
});

/* Upload instruments form */

const fileInput = document.getElementById('upload-instruments');
